...
import { ArrowLeft, CheckCircle, AlertCircle, Clock, Award, Loader2 } from 'lucide-react';
...
/**
 * Fixed Task Detail specifically for task IDs 1 and 2
 * This component bypasses the regular task loading mechanism
 * and uses direct hardcoded data to ensure reliable loading
 */
const FixedTaskDetail = () => {
  const { taskId } = useParams<{ taskId: string }>();
  const navigate = useNavigate();
  const { user } = useAuth();
  
  // Add console log to debug
  console.log('FixedTaskDetail: taskId =', taskId);
  ...
  const taskTitles: Record<string, string> = {
    ...
    '3': 'Science Test' // Added for completeness
  };
  ...
  useEffect(() => {
    const loadTaskData = async () => {
      try {
        ...
        // Validate taskId - we only support 1 and 2 in this fixed version
        if (!taskId) {
          console.error('FixedTaskDetail: No taskId provided in URL parameters');
          setError('No task ID provided in URL parameters');
          setIsLoading(false);
          return;
        }
        
        if (taskId !== '1' && taskId !== '2') {
          console.error(`FixedTaskDetail: Task with ID ${taskId} not supported by this page`);
          setError(`Task with ID ${taskId} not supported by this page`);
          setIsLoading(false);
          return;
        }
        
        // Get the fixed task data immediately without artificial delay
        const taskData = getFallbackTask(taskId);
        if (!taskData) {
        ...
        <main className="pt-24 px-4 pb-16 max-w-7xl mx-auto">
          <div className="bg-[#e8d5c4]/30 backdrop-blur-md rounded-xl shadow-md p-8 text-center border border-amber-200">
            ...
            <p className="text-gray-600 mb-6">
              This task page only supports task IDs 1 and 2 directly.
            </p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 to-white">
      ...
      <main className="pt-20 px-4 pb-16" style={{ background: "linear-gradient(to right, #dfd6c8, #e8d5c4)" }}>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    <div className="bg-white/20 px-3 py-1 rounded-full text-sm text-white">
                      {task?.subject || 'General Knowledge'}
                    </div>
                    <div className="bg-white/20 px-3 py-1 rounded-full text-sm text-white">
...
export default FixedTaskDetail; 